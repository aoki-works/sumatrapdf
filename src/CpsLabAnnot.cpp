...
struct MarkFileParser : json::ValueVisitor {
    ...
    bool pages(const char* path, const char* value);
    //
    bool Visit(const char* path, const char* value, json::Type type) override;
    void Parse(const char* path, const bool);
};

void MarkFileParser::Parse(const char* path, const bool sens) {
    this->sensitive = sens;
...
MarkerNode* MarkFileParser::getMark(const char* keyword) {
    ...
    for(auto m : markerTable) {
        if (this->sensitive) {
            ...
            if (str::EqI(m->keyword.Get(), keyword)) { return m; }
        }
    }
...
bool MarkFileParser::pages(const char* path, const char* value) {
    ...
    if (word_pages->find(name) == word_pages->end()) {
...
bool MarkFileParser::Visit(const char* path, const char* value, json::Type type) {
    if (json::Type::String == type) {
        if (this->mark_color(path, value))
            return true;
        if (this->select_color(path, value))
            return true;
        if (this->words(path, value))
            return true;
    } else if (json::Type::Number == type) {
        if (this->pages(path, value))
            return true;
    }
    return true;
}
...
class WordBlock {
    ...
    PdfColor             mark_color_;        // marker color
    PdfColor             select_color_;      // selector color

  public:
    WordBlock() : keyword_(), words_(nullptr), mark_color_(0xff00ffff), select_color_(0xff00ffff){
    }
    WordBlock(MarkerNode* kwd) : keyword_(), words_(nullptr), mark_color_(0xff00ffff), select_color_(0xff00ffff)
    {
        keyword_ = kwd->keyword.Get();
        words_ = new std::vector<node_t>;
        mark_color_ = kwd->mark_color;
        select_color_ = kwd->select_color;
    }
    ~WordBlock() {
        if (words_ != nullptr) {
            for (auto w : *words_) {
    ...
    std::string mark_color(void) {
        std::ostringstream hexs;
        hexs << "0x" << std::setfill('0') << std::setw(6) << std::hex << (0xffffff & mark_color_) << std::dec;
        return hexs.str();
    }
    std::string select_color(void) {
        std::ostringstream hexs;
        hexs << "0x" << std::setfill('0') << std::setw(6) << std::hex << (0xffffff & select_color_) << std::dec;
        return hexs.str();
    }
};
...
MarkerNode::MarkerNode(WindowTab* tab)
    : tab_(tab), filePath_(), user_area_(nullptr), keyword(), mark_color(0xff00ffff), select_color(0xff00ffff),
...
const char* MarkerNode::selectWord(MainWindow* win, const int pageNo, char* wd, bool conti) {
    ...
    const bool sens = dm->textSearch->GetSensitive();
    for (int i = 0; i < words.Size(); ++i) {
        char* mark_word = words.At(i);
        if (sens) {
            if (!str::Eq(wd, mark_word)) { continue; }
        } else {
...
bool MarkerNode::tExist(const int pageNo, const char* cell, const bool sens) {
    ...
    for (auto pno : pages) {
        if (pno == pageNo) {
            auto w = mark_words.at(i);
            if (sens) {
                if (str::Eq(cell, w)) { return true; }
            } else {
                if (str::EqI(cell, w)) { return true; }
            }
        }
        ++i;
    }
...
//
// =============================================================
Markers::Markers(WindowTab* tab) : tab_(), select_(), sense_(false), page_in_cell_() {
    tab_ = tab;
    select_ = (0x01 | 0x02 | 0x04);
    DisplayModel* dm = tab->AsFixed();
    if (dm != nullptr) {
        sense_ = dm->textSearch->GetSensitive();
    }
}
...
void Markers::sendSelectMessage(MainWindow* win, bool conti) {
                ...
                for (auto m : nodes) {
                    if (!m->selected_words.Contains(text)) {
                        ...
                        if (this->sense_) {
                            ...
                            if (m->keyword != nullptr && str::EqI(m->keyword, "Pin")) {
                                m->assoc_cells.Append(getCellsInPage(sel.pageNo));
                            }
                        }
                    }
    ...
    for (auto m : markerTable) {
        ...
        bool is_pin;
        if (this->sense_) {
            is_pin = (m->keyword != nullptr && str::Eq(m->keyword, "Pin"));
        } else {
            is_pin = (m->keyword != nullptr && str::EqI(m->keyword, "Pin"));
        }
...
void Markers::selectWords(MainWindow* win, StrVec& words) {
    ...
    bool conti = false;
    DisplayModel* dm = win->AsFixed();
    bool sens = dm->textSearch->GetSensitive();
    for (auto w : words) {
        ...
        if (is_pin) {
            ...
            int pageNo = -1;
            do {
                pageNo = cn->getPage(cellName.Get(), curPageNo, sens);
                if (0 < pageNo) {
                    if (pn->tExist(pageNo, pinName.Get(), sens)) {
            ...
            for (auto m : nodes) {
                ...
                if (!m->selected_words.Contains(s)) {
                    ...
                    if (sens) {
                        ...
                        if (markers != nullptr && m->keyword != nullptr && str::EqI(m->keyword, "Pin")) {
                            m->assoc_cells.Append(markers->getCellsInPage(pageNo));
                        }
                    }
                }
...
void SaveWordsToFile(MainWindow* win, const char* fname) {
    ...
    for (auto sel : word_vec) {
        if (sens) {
            ...
            if (prev == nullptr || !str::EqI(prev, sel)) {
                words.Append(sel);
                prev = sel;
            }
        }
...
loop_break:
    ...
    if (save_as != nullptr /* && have_page_numbers */ ) {
        ...
        if (err == 0) {
            ...
            // --------------------------------------------------
            int wcount = 0;
            std::fprintf(outFile, "   \"mark_color\" : {\n");
            for (auto bk : word_blocks) {
                if (wcount != 0) { std::fprintf(outFile, ",\n"); }
                std::fprintf(outFile, "       \"%s\" : \"%s\"", bk->keyword(), bk->mark_color().c_str());
                wcount += 1;
            }
            std::fprintf(outFile, "\n   },\n");
            // --------------------------------------------------
            wcount = 0;
            std::fprintf(outFile, "   \"select_color\" : {\n");
            for (auto bk : word_blocks) {
                if (wcount != 0) { std::fprintf(outFile, ",\n"); }
                std::fprintf(outFile, "       \"%s\" : \"%s\"", bk->keyword(), bk->select_color().c_str());
                wcount += 1;
            }
            std::fprintf(outFile, "\n   }");
            // ----------------------------------------------------
            for (auto bk : word_blocks) {
                ...
                for (auto w : *bk->words()) {
                    ...
                    std::fprintf(outFile, "    \"%ls\" : [", w.first.c_str());  // net-name, pin-name, etc
                    auto pages = w.second;
                    if (2 <= pages->size()) {
                        // sort page-number and erase duplicated page-numbers.
                    ...
                    for (size_t i = 0; i < pages->size(); i++) {
                        ...
                        std::fprintf(outFile, "%d", pages->at(i)); //page-number
                    }
                    std::fprintf(outFile, "]");
                    wcount += 1;
                }
